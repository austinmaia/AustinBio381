...
For my data, I utilized output files from contour tracing in the application Luscinia. Each row represents a given parameter (in this case, fundamental frequency) measured at 50 time points along the whistle's length. The raw data is not very easy to use in direct data analysis, so I first utilized a function to extract the start and end frequency, delta frequency, mean frequency, and standard deviation of each whistle. Standard deviation can possibly be used as a measure of whistle modulation (as it measures differences from the mean frequency for that particular whistle). From there, I utilized batch processing via a linear regression model to see if a relationship exists between the modulation/frequency standard deviation and the mean frequency. 
...
# outputs: list containing start value, end value, delta, mean and standard for each whistle of given parameter
###################################
